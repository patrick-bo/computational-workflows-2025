...
workflow{
    ...
    if (params.step == 7) {
        ...
        out_ch = in_ch.map{ [it.name, it.toString()]}.first()
    ...
    if (params.step == 11) {
        ...
        in_ch = channel.of([1, 'V'], [3, 'M'], [2, 'O'], [1, 'f'], [3, 'G'], [1, 'B'], [2, 'L'], [2, 'E'], [3, '33'])
         out_ch = in_ch.groupTuple().map { key, values -> [key, values] }
    }
    ...
    if (params.step == 12) {
        ...
        out_ch = left_ch.join(right_ch, by: 0)
    }
    ...
    if (params.step == 13) {
        ...
        even = in_ch.filter{ it % 2 == 0}
        odd = in_ch.filter{ it % 2 == 1}
        even.collect().view {"even numbers: $it"}
        odd.collect().view {"odd numbers: $it"}
        out_ch = in_ch.collect()
    ...
    if (params.step == 14) {
        ...
        in_ch = channel.of(
            ...
            in_ch.map { it.name }.collect()
            out_ch = in_ch.map { it.name }.collect()
            out_ch.collectFile(name: 'results/names.txt', newLine: true) { row ->
        row.join(',')
    }
    }
